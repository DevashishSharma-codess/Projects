...
const FALLBACK_HERO_IMAGE = "/card-bg-left.jpg";

// Show top 5 products in the sticky main showcase section
...
export function ProductShowcase() {
  ...
  // Synchronize state with URL parameter /products/:productId
  useEffect(() => {
    if (productId) {
      const idx = SHOWCASE_PRODUCTS.findIndex((p) => p.id === productId);
      if (idx !== -1) {
        setActiveIdx(idx);
  ...
  const openProductModal = (prod) => {
    const target = prod || SHOWCASE_PRODUCTS[activeIdx] || SHOWCASE_PRODUCTS[0];
    navigate(`/products/${target.id}`);
  };

  // Auto-scroll right product list so active item is centered
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      stRef.current = ScrollTrigger.create({
        ...
        pin: canvas,
        start: "top top",
        end: () => `+=${window.innerHeight * (SHOWCASE_PRODUCTS.length * 1.0)}`,
        pinSpacing: true,
        scrub: 0.5,
        anticipatePin: 1,
        ...
        onUpdate: (self) => {
          const totalProducts = SHOWCASE_PRODUCTS.length;
          ...
          // Direct DOM mutation for smooth line fill
  ...
  const activeProduct = SHOWCASE_PRODUCTS[activeIdx] || SHOWCASE_PRODUCTS[0];

  const handleSelectProduct = (targetIdx) => {
    const clamped = Math.min(SHOWCASE_PRODUCTS.length - 1, Math.max(0, targetIdx));
    activeIdxRef.current = clamped;
    setActiveIdx(clamped);

    if (stRef.current && window.innerWidth >= 640) {
      const st = stRef.current;
      const progressRatio = (clamped + 0.1) / SHOWCASE_PRODUCTS.length;
  ...
  const handleNext = () => {
    const nextIdx = (activeIdx + 1) % SHOWCASE_PRODUCTS.length;
    handleSelectProduct(nextIdx);
  };

  const handlePrev = () => {
    const prevIdx = (activeIdx - 1 + SHOWCASE_PRODUCTS.length) % SHOWCASE_PRODUCTS.length;
      ...
      >
        ...
        <div className="block sm:hidden w-full min-h-[90dvh] bg-[#fdfbf9] text-ink p-4 pt-14 pb-6 flex flex-col justify-between">
          ...
          <div className="flex items-center justify-between pb-3 border-b border-ink/10">
            <span className="font-mono text-[10px] uppercase tracking-widest text-ink/60 font-semibold">
              {String(activeIdx + 1).padStart(2, "0")} / {String(SHOWCASE_PRODUCTS.length).padStart(2, "0")} — FEATURED
            </span>
          ...
          <div className="my-auto py-4 flex flex-col gap-4">
            ...
            >
              <motion.img
                ...
                src={activeProduct.image || cardBg1}
                alt={activeProduct.name}
                onError={(e) => {
                  e.target.onerror = null;
                  e.target.src = cardBg1;
                }}
          ...
          {/* Carousel Indicator Dots & See All Products Button */}
          <div className="flex flex-col items-center gap-3 pt-1">
            ...
            <button
              ...
              className="w-full inline-flex items-center justify-center gap-2 bg-ink/5 hover:bg-ink hover:text-paper text-ink border border-ink/15 text-xs font-mono font-semibold py-2.5 rounded-full transition-all cursor-pointer active:scale-95 mt-1"
            >
              <Grid size={14} />
        ...
        >
          <div className="grid grid-cols-12 h-full w-full overflow-hidden m-0 p-0">
            ...
            <div className="lg:col-span-7 h-full w-full bg-[#0c0a08] p-6 sm:p-8 lg:p-10 xl:p-14 flex flex-col justify-between overflow-hidden relative border-r border-white/10">
              ...
              <div className="relative z-20 flex items-center justify-between text-white/90 pt-10 sm:pt-12">
                <span className="font-mono text-xs uppercase tracking-widest text-white/70">
                  ENACTON STUDIO / FEATURED PRODUCTS
                </span>
                ...
                <span className="font-mono text-xs text-white/70 bg-black/50 backdrop-blur-md px-3 py-1 rounded-full border border-white/15">
                  0{activeIdx + 1} / 0{SHOWCASE_PRODUCTS.length}
                </span>
              </div>
            ...
            {/* RIGHT COLUMN (lg:col-span-5 / 42%): FULL HEIGHT EVENLY DISTRIBUTED MENU FOR 5 PRODUCTS */}
            <div className="lg:col-span-5 h-full w-full bg-[#fdfbf9] p-6 sm:p-8 lg:p-10 xl:p-12 flex flex-col justify-between overflow-hidden">
              ...
              {/* Product List Menu - Spans Full Height evenly for 5 Products */}
              <div
                ref={listRef}
                className="flex-1 my-3 py-2 flex flex-col justify-between overflow-y-auto custom-scrollbar pr-1 scroll-smooth"
              >
                {SHOWCASE_PRODUCTS.map((prod, idx) => {
                  ...
                  return (
                    <div
                      ...
                      onClick={() => handleSelectProduct(idx)}
                      className={`group relative cursor-pointer py-3.5 sm:py-4 border-b border-ink/10 last:border-b-0 flex flex-col justify-center transition-all duration-300 ${
                        isActive ? "opacity-100 flex-grow" : "opacity-60 hover:opacity-90"
                      }`}
                    >
                      {/* Meta Index & Category */}
                      <div className="flex items-center justify-between font-mono text-[9px] sm:text-[10px] uppercase tracking-wider text-ink/50 mb-1">
                      ...
                      <h3
                        className={`font-outfit transition-colors duration-300 ${
                          isActive
                            ? "text-xl sm:text-2xl lg:text-3xl font-normal text-ink tracking-tight"
                            : "text-base sm:text-lg lg:text-xl font-light text-ink/60"
                        }`}
                      >
                        {prod.name}
                      </h3>
                      ...
                      <AnimatePresence mode="wait">
                        {isActive && (
                          ...
                          >
                            <p className="font-outfit text-xs sm:text-sm font-light text-ink/80 leading-relaxed pt-2 pb-1.5">
                              {prod.subhead || prod.description}
                            </p>

                            <div className="flex items-center justify-end pt-1 text-[10px] sm:text-xs font-mono text-ink/60">
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                      ...
                      {/* Progress Line */}
                      <div className="relative mt-3 h-[2.5px] w-full overflow-hidden rounded-full gpu-layer">
                        {isActive ? (
                          <div
              ...
              {/* Bottom Navigation Buttons & See All Products CTA */}
              <div className="pt-3 border-t border-ink/10 flex items-center justify-between text-xs text-ink/60 font-mono shrink-0 gap-2">
                <div className="flex items-center gap-2">
                  <button
                    onClick={handlePrev}
                    aria-label="Previous Product"
                ...
                <button
                  onClick={() => setIsAllProductsOpen(true)}
                  className="inline-flex items-center gap-1.5 px-4 py-2 rounded-full bg-ink text-paper text-xs font-mono font-medium hover:bg-black transition-all cursor-pointer shadow-md active:scale-95"
                >
                  <Grid size={13} />
                  <span>See All Products ({PRODUCTS.length})</span>
                  <ArrowRight size={13} />
